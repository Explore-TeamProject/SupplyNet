...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_profile') {
    ...
    if (empty($name) || empty($email)) {
        ...
        $stmt_check->bind_param("si", $email, $salesman_id);
        $stmt_check->execute();
        ...
        if ($res_check->num_rows > 0) {
            ...
            $stmt->bind_param("ssssi", $name, $email, $mobile, $address, $salesman_id);
            if ($stmt->execute()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_password') {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        if (password_verify($current_password, $user['Password'])) {
            ...
            $update_stmt->bind_param("si", $hashed_password, $salesman_id);
...
<head>
    ...
    <title>My Profile - SupplyNet</title>
    <!-- Fonts -->
    ...
    <style>
        ...
        .form-control:focus { border-color: var(--primary-color); box-shadow: 0 0 0 0.25rem rgba(54, 185, 204, 0.25); }
        ...
        .profile-header-bg { background: linear-gradient(135deg, var(--primary-color) 0%, #158596 100%); height: 120px; border-radius: 0.75rem 0.75rem 0 0; }
    ...
    <div class="topbar">
        <div class="d-flex align-items-center">
            ...
            <div class="ms-2 ms-md-0">
                <h4 class="m-0 fw-bold text-dark fs-5 fs-md-4">My Profile</h4>
                <p class="text-muted small mb-0 d-none d-md-block">Manage your operational identity and security parameters.</p>
            </div>
        </div>
            ...
            <div class="col-xl-4 col-lg-5">
                <div class="card card-custom h-100">
                    ...
                    <div class="profile-details-wrapper position-relative px-4">
                        <img src="https://ui-avatars.com/api/?name=<?php echo urlencode($current_user['UserName']); ?>&background=36b9cc&color=fff&size=200" alt="Profile Image" class="profile-img-lg mb-3">
                        ...
                        <div class="mt-3">
                            <span class="badge bg-primary bg-opacity-10 text-primary border border-primary-subtle px-3 py-2 fs-6 rounded-pill text-uppercase">
                                <i class="fas fa-briefcase me-2"></i><?php echo htmlspecialchars($current_user['Role']); ?>
                            </span>
                        </div>
...
<script>
    ...
    // Tab highlighting logic for Bootstrap Nav Pills (optional cleanups)
