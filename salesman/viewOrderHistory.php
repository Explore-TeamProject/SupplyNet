...
$has_placed_by = ($check_col && $check_col->num_rows > 0);
...
";
if ($has_placed_by) {
    $query .= " AND (o.placed_by = $salesman_id OR o.placed_by IS NULL) ";
}
...
<head>
    ...
    <title>Order History - SupplyNet</title>
    <!-- Fonts -->
    ...
    <div class="topbar">
        <div class="d-flex align-items-center">
            ...
            <div class="ms-2 ms-md-0">
                <h4 class="m-0 fw-bold text-dark fs-5 fs-md-4">Order History</h4>
                <p class="text-muted small mb-0 d-none d-md-block">View and track all your processed orders.</p>
            </div>
        </div>
        ...
        <div class="row align-items-center mb-3">
            <div class="col-md-7 mb-3 mb-md-0">
                <ul class="nav nav-pills flex-column flex-sm-row gap-2">
                    ...
                    ?>
                    <li class="nav-item">
                        <a class="nav-link <?php echo $status_filter === 'all' ? 'active' : 'bg-white text-dark shadow-sm border border-light'; ?>" href="viewOrderHistory.php?status=all<?php echo $qs; ?>" <?php echo $status_filter === 'all' ? 'style="background-color: var(--primary-color);"' : ''; ?>><i class="fas fa-list me-2"></i>All Orders</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link <?php echo $status_filter === 'pending' ? 'active' : 'bg-white text-dark shadow-sm border border-light'; ?>" href="viewOrderHistory.php?status=pending<?php echo $qs; ?>" style="<?php echo $status_filter === 'pending' ? 'background-color: #f6c23e;' : ''; ?>"><i class="fas fa-clock me-2"></i>Pending</a>
                    </li>
                    <li class="nav-item">
        ...
        <div class="card card-custom shadow-sm">
            <div class="card-header bg-white border-bottom py-3 d-flex flex-wrap justify-content-between align-items-center gap-3">
                <h6 class="m-0 font-weight-bold" style="color: var(--primary-color) !important;">
                    <i class="fas fa-list me-2"></i><?php echo $status_filter === 'all' ? 'Sales Records Archive' : ucfirst($status_filter) . ' Orders'; ?>
                </h6>
                <div class="d-flex align-items-center gap-2">
                    <div class="input-group input-group-sm rounded-pill" style="max-width: 250px; background: #f8f9fc; border: 1px solid #e3e6f0; overflow:hidden;">
                        ...
                        <input type="text" id="searchInput" class="form-control border-0 bg-transparent shadow-none px-1" placeholder="Search orders, clients...">
                    </div>
                    <a href="placeOrder.php" class="btn btn-sm btn-primary shadow-sm" style="background-color: var(--primary-color); border:none; border-radius: 5px;">
                            ...
                            <?php if (empty($orders)): ?>
                                <tr>
                                    <td colspan="6" class="text-center py-5">
                                        ...
                                        <h6 class="text-muted mt-3">No orders found in your history.</h6>
                                        <a href="placeOrder.php" class="btn btn-outline-primary mt-3 btn-sm shadow-sm rounded-pill px-4">Place First Order</a>
                                    </td>
                                </tr>
                            <?php else: ?>
                                        ...
                                        <td data-label="Date & Target">
                                            ...
                                            ?>
                                                <div class="small mt-1 fw-bold <?php echo $is_overdue ? 'text-danger' : 'text-danger opacity-75'; ?>">
                                                    ...
                                                    <?php if($is_overdue): ?>
                                                        <i class="fas fa-exclamation-triangle ms-1 fs-6 align-middle" title="Overdue!"></i>
                                                    <?php endif; ?>
                                                </div>
                                            <?php endif; ?>
                                        </td>
