...
// Handling Rejoin Action
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $action = $_POST['action'] ?? '';
    $target_user_id = intval($_POST['user_id'] ?? 0);
    
    if ($target_user_id > 0 && $action === 'rejoin') {
        $stmt = $conn->prepare("UPDATE Users SET markasdeleted=0 WHERE UserID=?");
        $stmt->bind_param("i", $target_user_id);
        if ($stmt->execute()) {
            $message = '<div class="alert alert-success d-flex align-items-center" role="alert"><i class="fas fa-check-circle me-3"></i> User has been successfully rejoined.</div>';
        } else {
            $message = '<div class="alert alert-danger d-flex align-items-center" role="alert"><i class="fas fa-exclamation-triangle me-3"></i> Failed to rejoin user.</div>';
        }
    }
}

// Fetch all DELETED non-admin users
$deleted_users = [];
$res = $conn->query("SELECT * FROM Users WHERE Role != 'admin' AND markasdeleted=1 ORDER BY UpdatedAt DESC");
if ($res) {
...
<head>
    ...
    <title>Deleted Users - SupplyNet Admin</title>
    <!-- Fonts -->
...
<div class="main-wrapper">
    ...
    <div class="topbar">
        <div class="d-flex align-items-center">
            ...
            <h4 class="m-0 fw-bold text-dark">Deleted Users Recovery</h4>
        </div>
    ...
    <div class="dashboard-content">
        ...
        <div class="mb-4">
            <a href="user_management.php" class="btn btn-secondary shadow-sm"><i class="fas fa-arrow-left me-2"></i>Back to Active Users</a>
        </div>
        ...
        <div class="card card-custom">
            <div class="card-header bg-danger text-white border-bottom py-3 d-flex justify-content-between align-items-center" style="border-radius: 0.75rem 0.75rem 0 0;">
                <h6 class="m-0 font-weight-bold" style="font-weight: 700;"><i class="fas fa-trash-alt me-2"></i>Deleted Users Archive</h6>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-hover align-middle border">
                        ...
                        <tbody>
                            <?php if(empty($deleted_users)): ?>
                                <tr><td colspan="5" class="text-center py-5 text-muted"><i class="fas fa-box-open d-block fs-1 mb-3 text-black-50 opacity-25"></i>No deleted users found in the archive.</td></tr>
                            <?php else: ?>
                                <?php foreach($deleted_users as $u): ?>
                                <tr>
                                    <td class="fw-semibold text-secondary">
                                        <div class="d-flex align-items-center">
                                            <div class="rounded-circle bg-danger bg-opacity-10 text-danger d-flex align-items-center justify-content-center me-2" style="width: 35px; height: 35px; font-weight:bold;">
                                                <?php echo strtoupper(substr($u['UserName'], 0, 1)); ?>
                                            </div>
                                            <span class="text-decoration-line-through text-muted"><?php echo htmlspecialchars($u['UserName']); ?></span>
                                        </div>
                                    </td>
                                    <td class="text-muted"><del><?php echo htmlspecialchars($u['Email']); ?></del></td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <form action="deleted_users.php" method="POST" class="d-inline mb-0">
                                            <input type="hidden" name="user_id" value="<?php echo $u['UserID']; ?>">
                                            <button title="Rejoin User" type="submit" name="action" value="rejoin" class="btn btn-action btn-success shadow-sm" onclick="return confirm('Restore this user account so they can log in again?');"><i class="fas fa-undo"></i></button>
                                        </form>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
